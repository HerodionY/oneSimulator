...
public class ProphetRouter extends ActiveRouter {
     ...
     * forwarding seed
     */
    private int forwardingSeed;

    /**
     * default value for the forwarding seed
     */
    public static final int DEFAULT_FORWARDING_SEED = 1;
    /**
     * forwarding seed -setting id ({@value})
     */
    public static final String FORWARDING_SEED_S = "forwardingSeed";
    /**
     * forwarding strategy -setting id ({@value})
     */
    public static final String FORWARDING_STRATEGY_S = "forwardingStrategy";
    /**
     * drop policy -setting id ({@value})
     */
	...
	/**
	 * Modified tryOtherMessages() method
	...
	/**
	 * MOPR – Evict the message with the highest delivery probability In an attempt to maximize the dispersion of messages through the network,
	 * this policy requires that the routing agent keeps track of the delivery probability of each message.
	 * The message with the highest delivery probability is the first to be dropped, thus giving messages that
    ...
    /**
     * Drop policies used to decide which message should be dropped when the
     * buffer is full.
     *
     * @author narwa
     * @see "Evaluation of Queueing Policies and Forwarding Strategies for
     * Routing in Intermittently Connected Networks" by Lindgren et al.
     */
    enum DropPolicy {
        FIFO(1, "FIFO"),
        SHLI(4, "SHLI"),
